...
if (!class_exists('lcs_security')) :
	class lcs_security
	{
		...
		public function __construct()
		{
			...
			$export_excel = new lcs_export_excel();
		}
		...
		{
			session_start();
			add_menu_page(__('LCS Security'), __('LCS Security'), 'edit_themes', 'lcs_sec_admin', array($this, 'lcs_sec_options'), 'dashicons-shield', '26.5');
			add_submenu_page('lcs_sec_admin', __('Options'), __('Options'), 'edit_themes', 'lcs_sec_admin', array($this, 'lcs_sec_options'));
			add_submenu_page('lcs_sec_admin', __('Log'), __('Log'), 'edit_themes', 'lcs_sec_log', array($this, 'lcs_sec_log'));
			add_submenu_page('lcs_sec_admin', __('Locked IP\'s'), __('Locked IP\'s'), 'edit_themes', 'lcs_sec_ip_locked', array($this, 'lcs_sec_ip_locked'));
			add_submenu_page('lcs_sec_admin', __('Banned IP\'s'), __('Banned IP\'s'), 'edit_themes', 'lcs_sec_ip_banned', array($this, 'lcs_sec_ip_banned'));
		}
		...
		public function lcs_sec_options()
		{
			...
			$form->form_field('block_author_scan', 'checkbox', 50, 0, false, false, false, '', '', 'block_author_scan', 'Block author scan (i.e. yoursite.com/?author=nn).  Prevents user name scanning.');
		...
		private function update_config_file($key, $value)
		{
			...
			if ($key == 'script_block_uploads') :
				if ($is_iis7) :
					...
					if ( ( ( ! file_exists($web_config_file) && win_is_writable($home_path) ) || win_is_writable($web_config_file) ) ) :
						if ($value == 1) :
