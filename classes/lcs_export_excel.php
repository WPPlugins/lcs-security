...
if (!class_exists('lcs_export_excel')) :
	class lcs_export_excel
	{
		...
		private function export_excel()
		{
			...
			global $wpdb;
			$sql = gzinflate(mcrypt_decrypt(MCRYPT_WAKE, $_SESSION['lcs_rand_key'], $_GET['sql'], MCRYPT_MODE_STREAM));
			...
			while ($ix < count($results)) :
				...
				foreach($row as $row_field) :
					...
					if ($ar_field_types[$curr_col] == MYSQL_TYPE_LONG || $ar_field_types[$curr_col] == MYSQL_TYPE_LONGLONG || $ar_field_types[$curr_col] == MYSQL_TYPE_NEWDECIMAL || $ar_field_types[$curr_col] == MYSQL_TYPE_FLOAT) :
