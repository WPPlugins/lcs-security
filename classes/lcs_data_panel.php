	...
	class lcs_data_panel
	{
		...
		public function data_show($sql, $ar_funcs = array(), $page_rows = 50) 
		{
			...
			if (empty($_SESSION['lcs_rand_key'])) :
				$_SESSION['lcs_rand_key'] = $this->generate_random_string(32);
			endif;
			//echo '<p>SQL: '.$sql.'</p>';
			...
			if ($wpdb->num_rows > 0) :
				...
				$ar_excel_url['sql'] = mcrypt_encrypt(MCRYPT_WAKE, $_SESSION['lcs_rand_key'], gzdeflate($sql,6), MCRYPT_MODE_STREAM);
				...
				$_SESSION['lcs_security_wpdb_'.$_SESSION['lcs_rand_key']] = json_encode($wpdb);
		...
		public function search_tokenize($str_fields, $int_fields, $search) 
		{
			...
			$ar_result = array();
			if (!empty($str_fields)) :
				$search_str = $wpdb->prepare("%s", '%'.$search.'%');
